...
class AnimeGANv3(object):
    def __init__(self, sess, args):
        ...
        self.val_real = tf.placeholder(
            tf.float32, [1, None, None, self.img_ch], name='val_input')

        self.real_photo = tf.placeholder(tf.float32, [
                                         self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='real_photo')
        self.photo_superpixel = tf.placeholder(tf.float32, [
                                               self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='photo_superpixel')

        self.fake_superpixel = tf.placeholder(tf.float32, [
                                              self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='fake_superpixel')
        self.fake_NLMean_l0 = tf.placeholder(tf.float32, [
                                             self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='fake_NLMean_l0')

        self.anime = tf.placeholder(tf.float32, [
                                    self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='anime_image')
        self.anime_smooth = tf.placeholder(tf.float32, [
                                           self.batch_size, self.img_size[0], self.img_size[1], self.img_ch], name='anime_smooth_image')
    ...
    def build_train(self):
        ...
        self.generated_s,  self.generated_m = self.generator(
            self.real_photo, is_training=True)
        self.generated = self.tanh_out_scale(guided_filter(self.sigm_out_scale(
            self.generated_s), self.sigm_out_scale(self.generated_s), 2, 0.01))  # 0.25**2

        """for val"""
        self.val_generated_s, self.val_generated_m = self.generator(
            self.val_real, is_training=False, reuse=True)
        self.val_generated = self.tanh_out_scale(guided_filter(self.sigm_out_scale(
            self.val_generated_s), self.sigm_out_scale(self.val_generated_s), 2, 0.01))  # 0.25**2

        # gray maping
        self.fake_sty_gray = tf.image.grayscale_to_rgb(
            tf.image.rgb_to_grayscale(self.generated))
        self.anime_sty_gray = tf.image.grayscale_to_rgb(
            tf.image.rgb_to_grayscale(self.anime))
        self.gray_anime_smooth = tf.image.grayscale_to_rgb(
            tf.image.rgb_to_grayscale(self.anime_smooth))

        # support
        fake_gray_logit = self.discriminator(self.fake_sty_gray)
        anime_gray_logit = self.discriminator(
            self.anime_sty_gray,  reuse=True, )
        gray_anime_smooth_logit = self.discriminator(
            self.gray_anime_smooth, reuse=True, )

        # main
        generated_m_logit = self.discriminator(
            self.generated_m, scope="discriminator_main")
        fake_NLMean_logit = self.discriminator(
            self.fake_NLMean_l0, reuse=True, scope="discriminator_main")

        """ Define Loss """
        # init G
        # + con_loss(self.real_photo, self.generated_m)
        self.Pre_train_G_loss = con_loss(self.real_photo, self.generated)

        # gan
        """support"""
        self.con_loss = con_loss(self.real_photo, self.generated, 0.5)

        self.s22, self.s33, self.s44 = style_loss_decentralization_3(
            self.anime_sty_gray, self.fake_sty_gray,  [0.1, 2.0,  28])
        ...
        self.G_support_loss = self.g_adv_loss + self.con_loss + \
            self.sty_loss + self.rs_loss + self.color_loss + self.tv_loss
        ...
        self.D_main_loss = discriminator_loss_m(
        ...
        with tf.control_dependencies(update_ops):
            self.init_G_optim = tf.train.AdamOptimizer(
                self.init_G_lr, beta1=0.5, beta2=0.999).minimize(self.Pre_train_G_loss, var_list=G_vars)
        ###
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
        ...
        """" Summary """
        #
        self.Summary_G_init_loss = tf.summary.scalar(
            "G_init", self.Pre_train_G_loss)
        #
        ...
        self.Summary_G_color_loss = tf.summary.scalar(
            "color_loss", self.color_loss)
        self.Summary_G_tv_loss = tf.summary.scalar("tv_loss", self.tv_loss)

        self.Summary_G_loss = tf.summary.scalar(
            "Generator_loss", self.Generator_loss)
        self.Summary_D_loss = tf.summary.scalar(
            "Discriminator_loss", self.Discriminator_loss)

        # ------
        self.pretrianed_G_merge = tf.summary.merge([self.Summary_G_init_loss])
        self.GD_loss_merge = tf.summary.merge([self.Summary_G_loss, self.Summary_G_adv, self.Summary_G_con_loss, self.Summary_G_rs_loss,
                                              self.Summary_G_sty_loss, self.Summary_G_color_loss, self.Summary_G_tv_loss, self.Summary_D_loss])
    ...
    def train(self):
        ...
        self.saver_load = tf.train.Saver(
            var_list=variables_to_resotre, max_to_keep=self.epoch)
        ...
        """ Input Image"""
        real_photo_op, anime_img_op, anime_smooth_op = self.real_generator.load_images(
        ...
        for epoch in range(start_epoch, self.epoch):
            for idx in range(steps):
                ...
                real_photo, anime_, anime_smooth_ = self.sess.run(
                    [real_photo_op, anime_img_op, anime_smooth_op])
                train_feed_dict = {
                ...
                if epoch < self.init_G_epoch:
                    _, init_loss, summary_str = self.sess.run(
                        [self.init_G_optim, self.Pre_train_G_loss, self.pretrianed_G_merge], feed_dict=train_feed_dict)
                    ...
                    """ Update G """
                    # output fake image
                    inter_out_s, inter_out = self.sess.run(
                        [self.generated_s, self.generated], feed_dict=train_feed_dict)
            ...
            if (epoch + 1) >= self.init_G_epoch:
                ...
                for i, sample_file in enumerate(val_files):
                    ...
                    val_real, test_, test_s, test_m = self.sess.run(
                        [self.val_real, self.val_generated, self.val_generated_s, self.val_generated_m], feed_dict={self.val_real: sample_image})
    ...
    def get_seg(self, batch_image):
        def get_superpixel(image):
            ...
            image_seg = segmentation.felzenszwalb(
                image, scale=5, sigma=0.8, min_size=100)

...
# He : tf.keras.initializers.VarianceScaling()
# Normal : tf.random_normal_initializer(mean=0.0, stddev=0.02)
# l2_decay : tf.keras.regularizers.l2(0.0001)


class TF1L2Regularizer(tf.keras.regularizers.Regularizer):
    def __init__(self, l2=0.0001):
        self.l2 = l2

    def __call__(self, x):
        return self.l2 * tf.math.reduce_sum(tf.math.square(tf.convert_to_tensor(x)))
...
weight_regularizer = TF1L2Regularizer(0.0001)
...
def layer_norm(x, scope=None):
    return tf.keras.layers.LayerNormalization(epsilon=1e-5, center=True, scale=True, name=scope)(x)


def batch_norm(x, is_training=True, scope=None):
    return tf.keras.layers.BatchNormalization(center=True, scale=True,
                                              momentum=0.999, epsilon=1e-3, name=scope,
                                              fused=True)(x, training=is_training)
    ...
    with tf.variable_scope(scope):
        ...
        if sn:
            ...
            x = tf.keras.layers.Conv2D(filters=int(channels),
                                       kernel_size=int(kernel), kernel_initializer=weight_init,
                                       kernel_regularizer=weight_regularizer,
                                       strides=int(stride), use_bias=use_bias)(x)
        return x
...
def Conv2D(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias=None, activation_fn=None):
    ...
    return tf.keras.layers.Conv2D(
        ...
        padding=padding,
        use_bias=True if Use_bias is not None else False)(inputs)
